/**
 * ch_usb_handler.c usb core handler of ch552g based keyboard firmware.
 * from CH55xDuino generic_examples.formly USBhandler.c.
 *
 *  created by Deqing Sun for use with CH55xduino 
 */

#include "ch_usb_handler.h"
#include "ch_usb_constant.h"

#ifndef USER_USB_RAM
#error "ch552_keypad_Library requires USER_USB_RAM to be defined in CH55xDuino board USB settings"
#endif
...
void USB_EP0_IN() {
	...
	__data uint8_t len ;
	case USB_GET_DESCRIPTOR:
		len = SetupLen >= DEFAULT_ENDP0_SIZE ? DEFAULT_ENDP0_SIZE : SetupLen; // send length
		for (__data uint8_t i = 0; i < len; i++)
			Ep0Buffer[i] = pDescr[i];
...
void USBInterrupt(void) { // inline not really working in multiple files in SDCC
	if (UIF_TRANSFER) { // Dispatch to service functions
		...
		case UIS_TOKEN_OUT:  // SDCC will take IRAM if array of function pointer is used.
			switch (callIndex) {
			case 0:
				EP0_OUT_Callback();
				break;
			...
			break;
		case UIS_TOKEN_SOF: 	// SDCC will take IRAM if array of function pointer is used.
			switch (callIndex) {
			case 0:
				EP0_SOF_Callback();
				break;
			...
			break;
		case UIS_TOKEN_IN: // SDCC will take IRAM if array of function pointer is used.
			switch (callIndex) {
			case 0:
				EP0_IN_Callback();
				break;
			...
			break;
		case UIS_TOKEN_SETUP: // SDCC will take IRAM if array of function pointer is used.
			switch (callIndex) {
			case 0:
				EP0_SETUP_Callback();
				break;
...
// CH559 use differend endianness for these registers
	...
	UEP0_CTRL = UEP_R_RES_ACK | UEP_T_RES_NAK; // Manual flip, OUT transaction returns
